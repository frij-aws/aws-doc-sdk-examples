...
CLASS ltc_awsex_cl_iot_actions IMPLEMENTATION.
  ...
  METHOD class_setup.
    ...
    cl_abap_unit_assert=>assert_bound( act = lo_sns_rsp
      msg = |class_setup: failed to create SNS topic| ).
    av_sns_topic_arn = lo_sns_rsp->get_topicarn( ).
    cl_abap_unit_assert=>assert_not_initial( act = av_sns_topic_arn
    ...
    cl_abap_unit_assert=>assert_bound( act = lo_role_rsp
      msg = |class_setup: failed to create IAM role| ).
    av_iam_role_arn = lo_role_rsp->get_role( )->get_arn( ).
    cl_abap_unit_assert=>assert_not_initial( act = av_iam_role_arn
    ...
    ao_iam->tagrole( iv_rolename = av_iam_role_name it_tags = lt_iam_tags ).
    DATA(lv_pol_doc) =
      '{"Version":"2012-10-17","Statement":[{' &&
  ...
  METHOD class_teardown.
    IF av_certificate_arn IS NOT INITIAL AND av_thing_name IS NOT INITIAL.
      TRY.
          ao_iot->detachthingprincipal(
            iv_thingname = av_thing_name iv_principal = av_certificate_arn ).
    ...
    IF av_certificate_id IS NOT INITIAL.
      TRY.
          ao_iot->updatecertificate(
            iv_certificateid = av_certificate_id iv_newstatus = 'INACTIVE' ).
    ...
    IF av_det_cert_id IS NOT INITIAL.
      TRY.
          ao_iot->updatecertificate(
            iv_certificateid = av_det_cert_id iv_newstatus = 'INACTIVE' ).
    ...
    IF av_del_cert_id IS NOT INITIAL.
      TRY.
          ao_iot->updatecertificate(
            iv_certificateid = av_del_cert_id iv_newstatus = 'INACTIVE' ).
      ...
      IF av_iam_policy_name IS NOT INITIAL.
        TRY.
            ao_iam->deleterolepolicy(
              iv_rolename = av_iam_role_name iv_policyname = av_iam_policy_name ).
          CATCH /aws1/cx_rt_generic.
        ENDTRY.
      ENDIF.
      TRY.
  ...
  METHOD create_thing.
    ...
    cl_abap_unit_assert=>assert_bound(
      act = lo_result msg = |create_thing returned no result| ).
    cl_abap_unit_assert=>assert_equals(
      exp = lv_new_thing act = lo_result->get_thingname( )
  ...
  METHOD list_things.
    ...
    ao_iot_actions->list_things( IMPORTING ot_things = lt_things ).

    cl_abap_unit_assert=>assert_not_initial(
      act = lt_things msg = |list_things returned no things| ).
    ...
    cl_abap_unit_assert=>assert_true(
      act = lv_found msg = |Shared thing '{ av_thing_name }' not found in list| ).
  ENDMETHOD.
  ...
  METHOD create_keys_and_certificate.
    ...
    ao_iot_actions->create_keys_and_certificate(
      IMPORTING ov_certificate_id = lv_cert_id ov_certificate_arn = lv_cert_arn ).

    cl_abap_unit_assert=>assert_not_initial(
      act = lv_cert_id msg = |Certificate ID is empty| ).
    cl_abap_unit_assert=>assert_not_initial(
  ...
  METHOD attach_thing_principal.
    ...
    ao_iot_actions->attach_thing_principal(
      EXPORTING iv_thing_name = lv_att_thing iv_principal = lv_att_cert_arn
    ...
    LOOP AT lo_princ_rsp->get_principals( ) ASSIGNING FIELD-SYMBOL(<lv_p>).
      IF <lv_p>->get_value( ) = lv_att_cert_arn.
        lv_attached = abap_true. EXIT.
      ENDIF.
    ENDLOOP.
    cl_abap_unit_assert=>assert_true( act = lv_attached
      msg = |Certificate was not attached to thing| ).

    ao_iot->detachthingprincipal(
      iv_thingname = lv_att_thing iv_principal = lv_att_cert_arn ).
    ao_iot->updatecertificate(
      iv_certificateid = lv_att_cert_id iv_newstatus = 'INACTIVE' ).
  ...
  METHOD describe_endpoint.
    ...
    ao_iot_actions->describe_endpoint( IMPORTING ov_endpoint_addr = lv_endpoint ).

    cl_abap_unit_assert=>assert_not_initial(
      act = lv_endpoint msg = |Endpoint address is empty| ).
  ...
  METHOD list_certificates.
    ...
    ao_iot_actions->list_certificates( IMPORTING ot_certificates = lt_certs ).

    cl_abap_unit_assert=>assert_not_initial(
      act = lt_certs msg = |list_certificates returned no certificates| ).
    ...
    LOOP AT lt_certs ASSIGNING FIELD-SYMBOL(<lo_cert>).
      ...
      ENDIF.
    ENDLOOP.
    cl_abap_unit_assert=>assert_true( act = lv_found
      msg = |Shared cert '{ av_certificate_id }' not found in list| ).
  ENDMETHOD.


  METHOD detach_thing_principal.
    cl_abap_unit_assert=>assert_not_initial( act = av_det_thing_name
      msg = |det-thing name not initialised| ).
    cl_abap_unit_assert=>assert_not_initial( act = av_det_cert_arn
      msg = |det-cert ARN not initialised| ).

    DATA lv_success TYPE abap_bool.
    ao_iot_actions->detach_thing_principal(
      EXPORTING iv_thing_name = av_det_thing_name iv_principal = av_det_cert_arn
      IMPORTING ov_success    = lv_success ).
    ...
    LOOP AT lo_princ_rsp->get_principals( ) ASSIGNING FIELD-SYMBOL(<lv_p>).
      IF <lv_p>->get_value( ) = av_det_cert_arn.
        lv_still_attached = abap_true. EXIT.
      ENDIF.
    ENDLOOP.
  ...
  METHOD delete_certificate.
    ...
    ao_iot_actions->delete_certificate(
      EXPORTING iv_certificate_id = av_del_cert_id
      IMPORTING ov_success        = lv_success ).
    ...
    TRY.
      ...
      CATCH /aws1/cx_rt_generic.
        lv_deleted = abap_true.
    ENDTRY.
    cl_abap_unit_assert=>assert_true( act = lv_deleted
  ...
  METHOD list_topic_rules.
    ...
    ao_iot_actions->list_topic_rules( IMPORTING ot_rules = lt_rules ).

    cl_abap_unit_assert=>assert_not_initial(
      act = lt_rules msg = |list_topic_rules returned no rules| ).
  ...
  METHOD update_indexing_configuration.
    DATA lv_success TYPE abap_bool.
    ao_iot_actions->update_indexing_configuration( IMPORTING ov_success = lv_success ).
  ...
  METHOD search_index.
    " Poll up to 90 s (18 × 5 s) for the index to become active.
    ...
    cl_abap_unit_assert=>assert_true( act = lv_ready
  ...
  METHOD delete_thing.
    cl_abap_unit_assert=>assert_not_initial( act = av_del_thing_name
      msg = |del-thing name was not initialised in class_setup| ).

    DATA lv_success TYPE abap_bool.
    ao_iot_actions->delete_thing(
      EXPORTING iv_thing_name = av_del_thing_name
      IMPORTING ov_success    = lv_success ).
  ...
  METHOD delete_topic_rule.
    cl_abap_unit_assert=>assert_not_initial( act = av_del_rule_name
      msg = |del-rule name was not initialised in class_setup| ).

    DATA lv_success TYPE abap_bool.
    ao_iot_actions->delete_topic_rule(
      EXPORTING iv_rule_name = av_del_rule_name
      IMPORTING ov_success   = lv_success ).
